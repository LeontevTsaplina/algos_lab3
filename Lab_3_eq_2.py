...
def gauss(func: Callable, x: list, y: list, eps: float = 0.001) -> (float, float):
    ...
    while (abs(a_next - a_cur) > eps) or (abs(b_next - b_cur) > eps) or (
            abs(func(x, y, a_next, b_next) - func(x, y, a_cur, b_cur)) > eps):
        a_cur = a_next
        b_cur = b_next

        results = []
        for a_i in a_vars:
            results.append(func(x, y, a_i, b_cur))

        a_next = a_vars[np.argmin(results)]

        results = []
        for b_i in b_vars:
            results.append(func(x, y, a_next, b_i))

        b_next = b_vars[np.argmin(results)]

        i += 1

    return a_next, b_next, i
...
def nelder_mead(func: Callable, eps: float = 0.001) -> (float, float):
    ...
    return best['x'][0], best['x'][1], best['nit']
...
# For equaling methods from lab 2 and lab 3
gss_lin = gauss(errors_func_lin, x_list, y_list)

print("GAUSS LINEAR")
print("A:", gss_lin[0])
print("B:", gss_lin[1])
print()
print("Number of iterations:", gss_lin[2])
print("Minimum function:", errors_func_lin(x_list, y_list, gss_lin[0], gss_lin[1]))
print("Best score:", errors_func_lin(x_list, y_list, gen_line_lin[0], gen_line_lin[1]))
print()

gss_rat = gauss(errors_func_ration, x_list, y_list)

print("GAUSS RATIONAL")
print("A:", gss_rat[0])
print("B:", gss_rat[1])
print()
print("Number of iterations:", gss_rat[2])
print("Minimum function:", errors_func_ration(x_list, y_list, gss_rat[0], gss_rat[1]))
print("Best score:", errors_func_ration(x_list, y_list, gen_line_ration[0], gen_line_ration[1]))
print()


nm_lin = nelder_mead(errors_func_lin_lev_marq_newton)

print("NELDER MEAD LINEAR")
print("A:", nm_lin[0])
print("B:", nm_lin[1])
print()
print("Number of iterations:", nm_lin[2])
print("Minimum function:", errors_func_lin(x_list, y_list, nm_lin[0], nm_lin[1]))
print("Best score:", errors_func_lin(x_list, y_list, gen_line_lin[0], gen_line_lin[1]))
print()

nm_rat = nelder_mead(errors_func_ration_lev_marq_newton)

print("NELDER MEAD RATIONAL")
print("A:", nm_rat[0])
print("B:", nm_rat[1])
print()
print("Number of iterations:", nm_rat[2])
print("Minimum function:", errors_func_ration(x_list, y_list, nm_rat[0], nm_rat[1]))
print("Best score:", errors_func_ration(x_list, y_list, gen_line_ration[0], gen_line_ration[1]))
print()